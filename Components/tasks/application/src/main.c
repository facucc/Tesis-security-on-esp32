...
#define PICTURE_UPLOAD_TOPIC "$aws/rules/UploadImages"
#define PICTURE_UPLOAD_TOPIC_LENGTH strlen(PICTURE_UPLOAD_TOPIC)

#define DELAY 300000

static const char *TAG = "applicationTask";
...
static void prvPrintImageBytes(const camera_fb_t *frame);
...
static void prSendPictureToAWS(const char * pictureEncoded, size_t pictureEncodedLength)
{
	//ESP_LOGI(TAG, "Size buffer encoded %d\n", pictureEncodedLength);
    ...
    PublishToTopic(PICTURE_UPLOAD_TOPIC,
                   PICTURE_UPLOAD_TOPIC_LENGTH,
                   buffer,
                   strlen(buffer),
                   MQTTQoS0,
                   TAG);
